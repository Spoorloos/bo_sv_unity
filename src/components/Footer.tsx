...
export default function Footer() {
    return (
        <footer className="px-[10%]">
            <div className="px-[5%] pb-8 border-b-2 border-border grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
                {Object.entries(links).map(([title, items], index) =>
                    <section key={index} className={index % 2 === 1 ? "sm:text-end lg:text-start" : ""}>
                ...
                <div className="grid sm:text-end lg:text-start">
                    ...
                    <section className="space-y-2 pt-6 sm:pt-0 mt-auto">
                        <h2 className="footer-column">Socials</h2>
                        <div className="h-7 space-x-3">
                            <Link className="h-full inline-block accent-hover" href={socials.instagram} aria-label="Instagram" title="Instagram"><Instagram className="inline h-full"/></Link>
                            <Link className="h-full inline-block accent-hover" href={socials.youtube} aria-label="Youtube" title="Youtube"><Youtube className="inline h-full"/></Link>
                            <Link className="h-full inline-block accent-hover" href={socials.facebook} aria-label="Facebook" title="Facebook"><Facebook className="inline h-full"/></Link>
                            <Link className="h-full inline-block accent-hover" href={socials.twitter} aria-label="Twitter" title="Twitter"><Twitter className="inline h-full"/></Link>
                        </div>
                    </section>
                </div>
            </div>
