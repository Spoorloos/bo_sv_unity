...
export default function Header() {
    ...
    return (
        <header className="mx-8 my-4 pb-2 flex items-center justify-between border-b-2 border-gray-300">
            ...
            <nav className={`${navEnabled ? 'flex bg-white fixed inset-0 flex-col items-start pt-24 p-[10%] text-3xl' : 'hidden sm:flex'} gap-4 font-kinetika`}>
                {Object.entries(tabs).map(([name, url], index) =>
                    <Link className={`transition-colors border-b-2 ${url === pathName ? "border-accent text-black" : "border-transparent text-gray-500 hover:text-black"} p-2 uppercase`} href={url} key={index} onClick={() => setNavEnabled(false)}>{name}</Link>
                )}
            </nav>
